...
public class AuthController {
    ...
    // 안에서 사용하는 변수명과 쿼리스트링의 키값이 다를 경우 괄호 안에 표기해주면 됨
    // 그리고 기본값도 설정이 가능함
    // 그리고 다른 타입도 가능하며 여러 개의 RequestParam도 받을 수 있다
    // int는 null을 허용 안 하기 때문에 값이 없음의 상태
    ...
    // 반환 => "검색 조건 - 이름 : ***, 이메일 : ***"
    @GetMapping("/search")
    public String searchUser(@RequestParam(required = false) String name, @RequestParam (value = "email", defaultValue = "no-email") String email) {
...
//        return ResponseEntity.ok().body(signinRespDto);
    ...
    // 응답 본문(JSON)에 "중복 여부"를 표시함.
    // 중복체크는 정상적인 요청에 대한 정상적인 응답이기 때문에 200 OK 이다.
    // 이메일이 중복이든 아니든 요청 자체는 정상적으로 처리됐기 때문에 400/409와 같은 에러코드를 주지 않는다.
    // 대신 JSON 응답 내부에서 중복됨/가능함을 구분
    // 그럼 언제 에러 코드 (409 Conflict)를 쓰느냐?
    // 그거는 진짜 예외 상황일 때
    // 중복된 이메일로 회원가입을 실제로 시도했을 때 이럴때 409
}
